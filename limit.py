...
def plotgaus(iFName,injet,iLabel,options):
    ...
    l.DrawLatex(0.15,0.82,'gen. pdf = %s(n_{#rho}=%i,n_{p_{T}}=%i)'%(options.pdf2, options.V1N2,options.V2N2))
    l.DrawLatex(0.15,0.75,'fit pdf = %s(n_{#rho}=%i,n_{p_{T}}=%i)'%(options.pdf1, options.V1N1,options.V2N1))
    ...
    if not options.justPlot:
        ...
        if ntoys<=100:
            ...
            for iT in range(nTimes):
                cmd_1 = 'combine -M GenerateOnly %s --rMax %s --rMin %s --toysFrequentist -t %i  --saveToys -n %s --freezeParameters %s -s %s --setParameters %s' % (base,options.rMax,options.rMin,100,baseName,options.freezeNuisances,options.seed+10+iT,options.setParameters)
                cmd_2 = 'cp higgsCombine%s.GenerateOnly.mH120.%s.root %s/'%(baseName,options.seed+10+iT,options.odir) 
                cmd_3 = 'combine -M GoodnessOfFit %s --rMax %s --rMin %s -t %i --toysFile %s/higgsCombine%s.GenerateOnly.mH120.%s.root --algorithm saturated -n %s --freezeParameters %s -s %s --setParameters %s' % (base,options.rMax,options.rMin,100,options.odir,baseName,options.seed+iT+10,baseName, options.freezeNuisances,options.seed+iT+10,options.setParameters)
                ...
                cmd_5 = 'combine -M GoodnessOfFit %s --rMax %s --rMin %s -t %i --toysFile %s/higgsCombine%s.GenerateOnly.mH120.%s.root --algorithm saturated -n %s --freezeParameters %s -s %s --setParameters %s' % (alt,options.rMax,options.rMin,100,options.odir,baseName,options.seed+iT+10,altName, options.freezeNuisances,options.seed+iT+10,options.setParameters)
...
def goodness(base,ntoys,iLabel,options):
    ...
    if not options.justPlot:
        ...
        if options.algo =='saturated':
             if ntoys <= 100:
                 ...
                 cmd_list=[]
                 for iT in range(nTimes):
                     cmd_this = 'combine -M GoodnessOfFit %s  --setParameterRange r=-20,20  --setParameters %s -s %i -t %i --toysFrequentist --algorithm %s -n %s --freezeParameters %s --redefineSignalPOIs r'% (base,options.setParameters,options.seed+iT+10, 100 ,options.algo,base.split('/')[-1].replace('.root',''),options.freezeNuisances)
                     print(cmd_this)
                     cmd_list.append(cmd_this)
                     #exec_me(' ( set -x ; combine -M GoodnessOfFit %s  --setParameterRange r=-20,20  --setParameters %s -s %i -t %i --toysFrequentist --algorithm %s -n %s --freezeParameters %s --redefineSignalPOIs r >&1) &'% (base,options.setParameters,options.seed+iT+10, 100 ,options.algo,base.split('/')[-1].replace('.root',''),options.freezeNuisances),options.dryRun)
                 if not options.dryRun:
                     procs_list = [subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE,  shell=True) for cmd in cmd_list]
                     for proc in procs_list:
                        proc.communicate()
                        proc.wait()
                 exec_me('sleep 1; wait', options.dryRun)
                 exec_me('hadd -k -f higgsCombine%s.GoodnessOfFit.mH120.%s.root higgsCombine%s.GoodnessOfFit.mH120.*.root'%(base.split('/')[-1].replace('.root',''), options.seed, base.split('/')[-1].replace('.root','')), options.dryRun)
        else:
    ...
    if options.dryRun and not options.justPlot: sys.exit()
...
if __name__ == "__main__":
    ...
    if options.method=='GoodnessOfFit':
        ...
        iLabel= 'bias_%s%i%i_vs_%s%i%i_%s%i'%(options.pdf1, options.V1N1, options.V2N1, options.pdf2, options.V1N2, options.V2N2, 
