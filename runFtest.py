...
if __name__ == "__main__":
    ...
    if not options.justPlot:    
        limit_cmd = 'python limit.py -M FTest --datacard %s --datacard-alt %s -o %s -n %i --p1 %i --p2 %i -t %i --lumi %f %s -r %f --seed %s --freezeNuisances %s --setParameters %s --V1N1 %s --V2N1 %s --V1N2 %s --V2N2 %s'%(datacardWS1,datacardWS2,toysDir, options.n, p1, p2, options.toys, options.lumi, dataString, options.r, options.seed, options.freezeNuisances,options.setParameters,options.V1N1,options.V2N1,options.V1N2,options.V2N2)
